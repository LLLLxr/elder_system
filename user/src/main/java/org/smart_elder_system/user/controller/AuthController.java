...
public class AuthController {
    ...
    @PostMapping("/refresh")
    public ResponseEntity<LoginVO> refreshToken(
    ...
    public ResponseEntity<VerifyResultVO> verifyIdCard(
            @Parameter(description = "身份证验证信息") @RequestBody IdCardVerifyDTO verifyDTO) {
        
        VerifyResultVO result = authService.verifyIdCard(verifyDTO);
    ...
    public ResponseEntity<VerifyResultVO> verifyFace(
            @Parameter(description = "人脸验证信息") @RequestBody FaceVerifyDTO verifyDTO) {
        
        VerifyResultVO result = authService.verifyFace(verifyDTO);
        return ResponseEntity.ok(result);
    }
