...
public class RoleController {
    ...
    public ResponseEntity<List<RolePo>> getRoleList(
            @Parameter(description = "关键词") @RequestParam(required = false) String keyword) {

        List<RolePo> roles = roleService.getRoleList(keyword);
    ...
    @PreAuthorize("hasAuthority('ROLE_MANAGE')")
    public ResponseEntity<List<RolePo>> getRolesWithPermissions() {

        List<RolePo> roles = roleService.getRolesWithPermissions();
        return ResponseEntity.ok(roles);
    }
    ...
    @PreAuthorize("hasAuthority('USER_VIEW')")
    public ResponseEntity<List<PermissionPo>> getUserPermissions(
            @Parameter(description = "用户ID") @PathVariable Long userId) {

        List<PermissionPo> permissions = roleService.getPermissionsByUserId(userId);
    ...
    @PreAuthorize("hasAuthority('ROLE_MANAGE')")
    public ResponseEntity<List<RolePo>> getRolesByPermissionCode(
            @Parameter(description = "权限编码") @RequestParam String permissionCode) {

        List<RolePo> roles = roleService.getRolesByPermissionCode(permissionCode);
        return ResponseEntity.ok(roles);
    }
}
