...
import {
  ...
  CARE_JOURNEY_INTAKE_RECORDS_BY_APPLICANT_PATH,
  CARE_JOURNEY_INTAKE_RECORDS_PATH,
  CARE_JOURNEY_LATEST_RESULT_BY_APPLICANT_PATH,
...
export async function continueJourneyAfterAssessment(
  ...
  const url = buildPostUrl(CARE_JOURNEY_CONTINUE_PATH, {
...
export async function listIntakeRecords(elderId: number): Promise<IntakeRecord[]> {
  const response = await apiClient.get(CARE_JOURNEY_INTAKE_RECORDS_PATH, {
    params: { elderId },
  });
  return unwrapApiData<IntakeRecord[]>(response.data);
}

export async function listIntakeRecordsByApplicant(applicantName: string): Promise<IntakeRecord[]> {
  const response = await apiClient.get(CARE_JOURNEY_INTAKE_RECORDS_BY_APPLICANT_PATH, {
...
export async function getLatestJourneyResultByApplicant(applicantName: string): Promise<ServiceJourneyResult> {
  const response = await apiClient.get(CARE_JOURNEY_LATEST_RESULT_BY_APPLICANT_PATH, {
    params: { applicantName },
  });
  return unwrapApiData<ServiceJourneyResult>(response.data);
}
